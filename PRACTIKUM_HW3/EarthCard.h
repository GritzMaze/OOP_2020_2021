...
private:
    ...
    void copy(const EarthCard &other)
    {
        setName(other.name);
...
public:
    ...
    EarthCard(const String &o_name, const size_t &o_power = 0, const size_t &o_bonus = 0)
    {
        name = o_name;
        power = o_power;
        bonus = o_bonus;
    ...
    EarthCard(const EarthCard &other)
    {
        copy(other);
    }
    ...
    friend std::ostream &operator<<(std::ostream &out, const EarthCard &other)
    {
    ...
    friend std::istream &operator>>(std::istream &in, EarthCard &other)
    {
        std::cout << "Enter name: ";
        in >> other.name;
